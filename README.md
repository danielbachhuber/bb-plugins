...
| `bb-plugin-dynamic-ui` | `dynamic-ui` | Lets a skill show its results as a list above the thread's composer, each item opening in the side panel with its details and buttons: send a reply to the thread, open a new thread, run a confirmed command, or open a link. |
...
| `bb-plugin-now` | `now` | One list of what needs doing now, from Todoist and your Gmail inbox, with GitHub notifications gathered per pull request or issue. Complete tasks, archive email, snooze anything, reply on GitHub, and start a thread from any row. |
...
| `bb-plugin-tokenomics` | `tokenomics` | Graphs how many tokens your threads used over the past day, three days, or week, lists each thread's share, and puts a sparkline of each thread's turns in its header that opens a summary of where its tokens went. |
